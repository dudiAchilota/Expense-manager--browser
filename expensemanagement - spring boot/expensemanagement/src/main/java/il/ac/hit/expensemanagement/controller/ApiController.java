...
public class ApiController  {
    IModelExpenseDAO dao = null;

    static public final String file = "/view/api/";
    static public final String ERR_MSG_KEY = "errMsg";
    ...
    public void items(HttpServletRequest request, HttpServletResponse response) {
        try {
            String user = (String) request.getSession().getAttribute("user");
            if (user == null) {
                request.getRequestDispatcher(file + "login.jsp").forward(request, response);
            } else {
